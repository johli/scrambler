from utils.pytorch_fixes_sequence import *
from torch.nn import functional as F
import torch.nn as nn
from torch.nn import Module
from sal.utils.mask_sequence import *
...
def get_black_box_fn(model_zoo_model=resnet50, cuda=True, image_domain=(-2., 2.)):
    ...
    def black_box_fn(_images):
        if image_domain is not None :
            ...
            return black_box_model(_images)
    return black_box_fn
...
class SaliencyModel(Module):
    ...
    def restore(self, save_dir):
        p = os.path.join(save_dir, 'model-%d.ckpt' % 1)
        _ = self.load_state_dict(torch.load(p))
        
    def save(self, save_dir):
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        torch.save(self.state_dict(), os.path.join(save_dir, 'model-%d.ckpt' % 1))
    ...
    def forward(self, _images, _selectors=None, pt_store=None, model_confidence=0.):
        ...
        if self.allow_selector:
            ...
            act = torch.sum(main_flow*em.view(-1, int(em.shape[1]), 1, 1), 1, keepdim=True)
