...
from sal.datasets import optimus5_synth_dataset
...
class CNNClassifier(nn.Module) :
    ...
    def __init__(self, batch_size, padding=0) :
...
model_pytorch = CNNClassifier(batch_size=32, padding=14)
_ = model_pytorch.load_state_dict(torch.load("../../scrambler_pytorch_optimus5/optimusRetrainedMain_pytorch.pth"))
...
model_save_str = "pytorch_saliency_model_optimus5_synth_smaller_blur_less_penalty_resnet50_n_epochs_phase1_" + str(n_epochs_phase_1) + "_phase2_" + str(n_epochs_phase_2)
...
blur_kernel_size = 7#55
blur_sigma = 3#11

saliency_p = nn.DataParallel(saliency).cuda()
saliency_loss_calc = SaliencyLoss(black_box_fn, area_loss_coef=1, smoothness_loss_coef=0.005, num_classes=2, blurred_version_prob=blurred_version_prob, blur_kernel_size=blur_kernel_size, blur_sigma=blur_sigma) # model based saliency requires very small smoothness loss and therefore can produce very sharp masks
