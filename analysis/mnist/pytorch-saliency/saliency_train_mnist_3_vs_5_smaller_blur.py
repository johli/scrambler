...
from sal.datasets import mnist_dataset_3_vs_5
...
model_save_str = "pytorch_saliency_model_mnist_3_vs_5_smaller_blur_resnet50_n_epochs_phase1_" + str(n_epochs_phase_1) + "_phase2_" + str(n_epochs_phase_2)
...
saliency_loss_calc = SaliencyLoss(black_box_fn, smoothness_loss_coef=0.005, num_classes=10, blurred_version_prob=blurred_version_prob, blur_kernel_size=blur_kernel_size, blur_sigma=blur_sigma) # model based saliency requires very small smoothness loss and therefore can produce very sharp masks
...
def ev_phase1(_images, _labels):
    ...
    _targets = (_labels + Variable(torch.Tensor(_images.size(0)).uniform_(1, 9).cuda()).long()*__fakes.long())%10
...
def ev_phase2(_images, _labels):
    __fakes = Variable(torch.Tensor(_images.size(0)).uniform_(0, 1).cuda()<FAKE_PROB)
    _targets = PT(targets=(_labels + Variable(torch.Tensor(_images.size(0)).uniform_(1, 9).cuda()).long()*__fakes.long())%10)
    _is_real_label = PT(is_real_label=(_targets == _labels).long())
    _masks, _exists_logits, _ = saliency_p(_images, _targets)
    PT(exists_logits=_exists_logits)
...
def phase2_visualise(s):
    ...
    orig = auto_norm(pt['images'][0]) / 256.0 #Johannes Fix
